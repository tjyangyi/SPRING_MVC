(function ($, window, undefined) {
    ...
    (function (style) {
        if (!style) {
            ...
            link.href = _path + "../css/jquery.dialog.css";
            link.rel = "stylesheet";
    ...
    var lhgdialog = function (config) {
        ...
        config.ok && config.button.push({id: "ok", name: config.okVal, callback: config.ok, focus: config.focus});
        config.cancel && config.button.push({id: "cancel", name: config.cancelVal, callback: config.cancel});
    ...
    lhgdialog.fn = lhgdialog.prototype = {
        constructor: lhgdialog, _init: function (config) {
            ...
            if (config.icon && !isIfr) {
                ...
                DOM.icon[0].innerHTML = '<img src="resource/comm/images/' + config.icon + '" class="_bg"/>'
            } else {
        ...
        /*! 关闭对话框 */
        close: function () {
            var that = this, DOM = that.DOM, wrap = DOM.wrap, list = lhgdialog.list, fn = that.config.close, afterClose = that.config.afterClose;
            that.time();
            if (that.iframe) {
        ...
        /*! 最大化窗口 */
        max: function () {
            var that = this, maxSize, currentSize = {
                w: '',
                h: ''
            }, DOM = that.DOM, wrapStyle = DOM.wrap[0].style, mainStyle = DOM.main[0].style, rbStyle = DOM.rb[0].style, titleStyle = DOM.title[0].style, config = that.config, top = _$top.scrollTop(), left = _$top.scrollLeft();
            if (!that._maxState) {
        ...
        _getDOM: function () {
            ...
            var name, i = 0, DOM = {wrap: $(wrap)}, els = wrap.getElementsByTagName("*"), len = els.length;
            for (; i < len; i++) {
        ...
        _ie6PngFix: function () {
            if (_ie6) {
                var i = 0, elem, png, pngPath, runtimeStyle, path = "resource/comm/images/", list = this.DOM.wrap[0].getElementsByTagName("*");
                for (; i < list.length; i++) {
                    elem = list[i];
        ...
        _addEvent: function () {
            ...
            DOM.wrap.bind("click", function (event) {
                ...
                if (target === DOM.close[0]) {
                    ...
                    if (target === DOM.max[0] || target === DOM.res[0] || target === DOM.max_b[0] || target === DOM.res_b[0] || target === DOM.res_t[0]) {
                        that.max();
                        return false
                    } else {
                        if (target === DOM.min[0] || target === DOM.min_b[0]) {
                            that.min();
                            return false
                        } else {
    ...
    lhgdialog.setting = {
        ...
        data: null,
        extendDrag: false,
        onBeforMaxSize: function (w, h, dom) {
        }
    };
    /*!
     *------------------------------------------------
    ...
    lhgdialog.dragEvent = {
        onstart: _fn, start: function (event) {
    ...
    _use = function (event) {
        var limit, startWidth, startHeight, startLeft, startTop, isResize, api = lhgdialog.focus, config = api.config, DOM = api.DOM, wrap = DOM.wrap[0], title = DOM.title, main = DOM.main[0], _dragEvent = lhgdialog.dragEvent, clsSelect = "getSelection" in _top ? function () {
        ...
        _dragEvent.onmove = function (x, y) {
            if (isResize) {
                ...
                config.onBeforMaxSize.apply(this, [width, height, DOM.main[0]]);
        ...
        limit = (function (fixed) {
            var ow = wrap.offsetWidth, oh = title[0].offsetHeight || 20, ww = _$top.width(), wh = _$top.height(), dl = fixed ? 0 : _$top.scrollLeft(), dt = fixed ? 0 : _$top.scrollTop();
            maxX = ww - ow + dl;
            maxY = wh - oh + dt;
    ...
    $(function () {
        setTimeout(function () {
            ...
            lhgdialog({left: "-9999em", time: null, fixed: false, lock: false, focus: false})
        }, 150);
...
(function ($, lhgdialog, undefined) {
    ...
    lhgdialog.tips = function (content, time, icon, callback) {
        var reIcon = icon ? function () {
            this.DOM.icon[0].innerHTML = '<img src="resource/comm/images/' + icon + '" class="ui_icon_bg"/>';
